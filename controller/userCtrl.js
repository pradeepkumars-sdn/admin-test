      ...
      if (checkEmail) {
        ...
        await userData.save().then((err, result) => {
          if (err) {
            ...
            res.status(200).json({ message: "Added", data: result });
          }
...
async function login(req, res) {
  ...
  if (user.isDeleted) {
    return res.status(200).json({message :"User is not available or deleted"})
  }
  ...
  if (passwordIsValid) {
    ...
    return res.status(200).json({message:"Login Successfully", resObject})
    //response.successResponse(res, 200, resObject, MESSAGE.LOGIN_SUCCESS);
  } else {
